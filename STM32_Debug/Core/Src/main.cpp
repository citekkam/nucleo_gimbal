...
void DWT_Init(void) {
    // Povolení trasovací jednotky
    CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
    // Resetování čítače cyklů
    DWT->CYCCNT = 0;
    // Povolení čítače cyklů
...
int main(void)
{
  ...
  //--------------------------------ODrive--------------------------
    ...
    sFilterConfig.FilterMaskIdLow = 0x0000; // Accept all messages
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  */
	  		  ...
	  		  if (odrv0_user_data.received_feedback )
	  		  {
	  			  ...
	  			  error_position = feedback.Pos_Estimate + ((-reference_angle/360)+(filter.angle/360.0));
	  			  //printf("Pos_Estim: %.4f reference: %.4f target: %.4f \r\n",feedback.Pos_Estimate, (-reference_angle/360)+(filter.angle/360.0), filter.angle);
	  	  	      //odrv0.setPosition(error_position, 0, 0);
	  			  //printf("gyro: %.4f, angle: %.4f \r\n", filter.angle, deg_XZ);
	  	  	  ...
	  	  	  if ((HAL_GetTick() - last_print_time) >= 1000)
	  	        {
	  	            ...
	  	            //printf("ODrive Rate: %lu\r\n", count_limit);
	  	            printf("ODrive Rate: %d\r\n", count_limit);
	  	            count_limit = 0;
...
int __io_putchar(int ch) {
    // Ensure &huart2 is the correct handle
    HAL_UART_Transmit(&huart2, (uint8_t *)&ch, 1, HAL_MAX_DELAY);
    return ch;
}

// Just in case the library tries to call _write directly:
