                                ...
                                icm_gyro_fs_t fs)
{
    ...
    /*
    // Wake + set clock to PLL (X-gyro): SLEEP=0, CLKSEL=1
    v = 0x01;
    st = HAL_I2C_Mem_Write(&hi2c1, dev, REG_PWR_MGMT_1, I2C_MEMADD_SIZE_8BIT, &v, 1, 20);
    if (st != HAL_OK) return st;
    HAL_Delay(50); // let PLL lock

    // CONFIG: set DLPF (bits 2:0). Example: 2=94 Hz, 3=44 Hz
    v = (dlpf_cfg & 0x07);
    st = HAL_I2C_Mem_Write(&hi2c1, dev, REG_CONFIG, I2C_MEMADD_SIZE_8BIT, &v, 1, 20);
    if (st != HAL_OK) return st;

    // Sample rate divider (output = 1kHz/(1+div) when DLPF!=0)
    v = smpl_div;
    st = HAL_I2C_Mem_Write(&hi2c1, dev, REG_SMPLRT_DIV, I2C_MEMADD_SIZE_8BIT, &v, 1, 20);
    if (st != HAL_OK) return st;

    // Gyro range: FS_SEL in bits 4:3
    // Read-modify-write to keep other bits
    st = HAL_I2C_Mem_Read(&hi2c1, dev, REG_GYRO_CONFIG, I2C_MEMADD_SIZE_8BIT, &v, 1, 20);
    if (st != HAL_OK) return st;
    v &= ~0x18;                 // clear FS_SEL
    v |= (uint8_t)(fs << 3);    // set FS
    st = HAL_I2C_Mem_Write(&hi2c1, dev, REG_GYRO_CONFIG, I2C_MEMADD_SIZE_8BIT, &v, 1, 20);
    if (st != HAL_OK) return st;

    // (Optional) you can set accel range/filters similarly if needed.
...
void Read_Gyro() {
	...
	/*
	 *
	sprintf(msg, "X %d Y %d Z %d %d %d\r\n",(int)rad_x, (int)rad_y, (int)rad_z, iic_err, count_limit);
	HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), 100);
	 */
}
...
void Read_Data_ACC()
{
	...
	/*
	 *
	sprintf(msg, "X: %d, Z: %d, DEG: %d %d\r\n", (int)acc_x, (int)acc_z, (int)deg_XZ, iic_err);
	HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), 100);
	 */
}
...
void Combine_sensor()
{
	 ...
	 *
	int16_t acc_x_raw, acc_y_raw, acc_z_raw;
	float acc_x, acc_y, acc_z, rad_XZ, deg_XZ;
	int16_t gyro_x_raw, gyro_y_raw, gyro_z_raw;
	...
	//rad_XZ = atan2f(-acc_x, sqrtf(acc_y*acc_y + acc_z*acc_z));

	rad_XZ  = (atan2f(acc_x, acc_z));
	deg_XZ = (rad_XZ*RAD2DEG);

	ret = HAL_I2C_Mem_Read(&hi2c1, (ICM20602_ADDRESS<<1) + 1, REG_DATA_GYRO, 1, data, 6, 100);
	if (ret != HAL_OK) {
		iic_err += 1;
	}
	gyro_x_raw = (int16_t)((data[0] << 8) + data[1]);
	...
	degree = (alpha * rad_y) + ((1.0 - alpha) * (-deg_XZ));
	sprintf(msg, "degree: %d ACC: %d GYRO: %d %d\r\n", (int)degree, (int)deg_XZ, (int)rad_y, iic_err);
	HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), 100);
	 */
}
